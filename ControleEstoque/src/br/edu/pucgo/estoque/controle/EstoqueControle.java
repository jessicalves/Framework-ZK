...
public class EstoqueControle extends SelectorComposer<Window> {
	...
	public void onClickbtnLimparCampos() throws SQLException, InterruptedException, IOException {
		this.limpaDados();
	}
	...
	public void onBlurtxtPesquisa()
		...
		if (aux.length() == 0 || aux.isEmpty() || aux.equals("") || aux == null) {
			...
			sql = "SELECT * FROM produto WHERE nome like '%" + aux + "%' ORDER BY prod_id ASC";
		}
	...
	public void obtemPessoaSelecionada() throws SQLException, ClassNotFoundException, InterruptedException {
		...
		if (indice >= 0) {
			...
			if (produto != null) {
				...
				this.txtPrecoCompra.setText((Double.toString(produto.getPreco_compra())));
				this.txtPrecoVenda.setText((Double.toString(produto.getPreco_venda())));
				this.txtQtde.setText(Integer.toString(produto.getQtde_estoque()));
				;
				this.limpaLsbPesquisa();
				this.txtPesquisa.setText("");
